...
| Supabase 프로젝트 | "sabol channel" (ref `qmvsacuofsfamvqnvnrm`) — **버튜버 프로젝트와 별개** |
| 서버 | Oracle Cloud, **Japan East (Osaka)**, Ubuntu 22.04, `VM.Standard.E2.1.Micro` (Always Free) |
| 서버 공인 IP | `64.110.110.112` (임시 IP — 인스턴스 재생성 시 바뀜) |
| SSH 유저 | `ubuntu` (키: 로컬 `Desktop/홍보/ssh-key-...key`) |
...
**SSH 접속**
```bash
ssh -i <키경로> ubuntu@64.110.110.112
```
