...
## 5. "같은 WiFi가 아니어도 되나?" → 된다

지금 쓰는 `http://192.168.0.3:8188`은 **집 공유기 안에서만 통하는 사설 IP**라 같은 WiFi에서만 연결된다.
Tailscale을 깔면 이 제약이 사라진다.

| | 주소 | 어디서 되나 |
|---|---|---|
| 지금 | `192.168.0.3:8188` | **같은 WiFi만** |
| Tailscale | `https://내PC.테일넷.ts.net` | **어디서든** (LTE·5G·다른 WiFi·해외) |

Tailscale은 내 기기만 묶는 개인 전용 암호화 네트워크(메시 VPN)다. 폰·PC가 어디 있든 서로 통신하고
**주소가 고정**된다. 폰에서는 **Tailscale 앱 VPN 토글 ON** 한 번만 하면 된다.

### ⚠️ 두 문제는 별개다
| 문제 | 해결책 |
|---|---|
| 밖에서 연결이 안 됨 | **Tailscale** |
| PC가 꺼져 있음 | **스마트 플러그** |

**Tailscale은 꺼진 PC를 켜주지 못한다.** 그래서 둘을 함께 쓴다.

### 다른 방법
- **Cloudflare Tunnel**: 폰에 앱 없이 https 주소로 바로 접속. 단 **주소를 아는 누구나 접근 가능**
  → 성인 콘텐츠 사용 환경에선 Tailscale이 더 안전. (무료 Cloudflare Access로 이메일 인증 추가는 가능)
- **iptime 포트포워딩**: ❌ 비추천. 집 IP를 인터넷에 노출 → ComfyUI가 외부 스캔에 노출된다.
